	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $link => $site) {
      	# code...
			...
			elseif ($platform=="faq.whatsapp") {
					# code...
				echo ''.$site['faq.whatsapp'].'';
			}
...
function getblogridposts($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $bloggrid => $griditem) {
      	# code...
			echo '<div class="col-md-12 blog-grid-top">
			<div class="b-grid-top">
			<div class="blog_info_left_grid">
			<a href="single.php?id='.$griditem['id'].'">
			<img src="blogadmin/images/'.$griditem['photo'].'" class="img-fluid" alt="fantastic cms" style="width:120px;height:90px">
			</a>
			</div>
			<h2 style="font-family:calibri; font-size:20px; color:black;">
			...
			<a href="#" class="far fa-clock">'.$griditem['date'].'</a>
		
			<a href="#" class="far fa-user"> '.$griditem['author'].'</a>
		
			<a href="#" class="fas fa-tags">'.$griditem['tags'].'</a>
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $thefour => $fourdata) {
      	# code...
			...
			<img src="blogadmin/images/'.$fourdata['photo'].'" alt="fantastic cms" data-desoslide-caption="<h5>Latest Post '.$fourdata['id'].'</h5>">
			<div class="mid-text-info">
			<h4 style="height:20px;overflow:hidden;text-overflow:ellipsis">'.$fourdata['title'].'</h4>
			<p>'.$fourdata['author'].'</p>
			<div class="sub-meta">
			<span>
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $edschoice => $choice) {
			...
			<a href="single.php?id='.$postdata['id'].'">'.$postdata['title'].'</a>
								...
								<a href="single.php?id='.$postdata['id'].'" class="btn btn-primary read-m">Read More</a>
							</div>';
				}
			}
      	# code...
				...
				foreach ($result as $allblogs => $specificblog) {
					...
					echo '<div class="blog-grids row mb-3">
							...
							<div class="col-md-7 blog-grid-right">
								...
								<h5>
									<a style="color:yellow;" href="single.php?id='.$specificblog['id'].'">'.$specificblog['title'].' </a>
								</h5>
								<div class="sub-meta">
									<span>
